...
for i in 0..pythonfiles.size - 1
	/Generate a random number between one and two/
 	if (rand(1..2) == 1)
 		...
 		File.delete("#{Dir.getwd}/#{pythonfiles[i]}") if File.exists?("#{Dir.getwd}/#{pythonfiles[i]}")
 	else
 		/Declare and initialize a string/
 		text = ""
 		/Open the file of element i in the pythonfiles array with reading-mode/
 		File.open("#{Dir.getwd}/#{pythonfiles[i]}", "r") do |file|
 			/While there is a line to get it adds it to the text variable/
 			while line = file.gets
 				text << line
 			end
 			/Passes the text string as a argument and translates it to rovarsproket/
